...
In general, `⎕DQ` first updates the screen with any pending changes, then hands control to the user and waits for an event. If its argument is `#`, `'#'`, or `'.'`, `⎕DQ` processes events for all active objects, i.e. for those objects and their children whose Active property is 1. If the right argument specifies one or more top-level objects (such as Form, PropertySheet), `⎕DQ` processes events for all of these objects and their children, and (if the current thread is thread 0) for the Root object, but ignores any others, even though they may be currently active.
