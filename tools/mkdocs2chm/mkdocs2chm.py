...
def static_assets(assets_dir, project='project') -> Tuple[List[str], str]:
    """
    ...
    for root, _, files in os.walk(assets_dir):
        for file in files:
            print(f'Checking asset {file}')
            rel_name = os.path.relpath(os.path.join(root, file), start=assets_dir)
            newname = os.path.join(project, 'assets', rel_name)
            ...
            if newname.endswith('.css'):
                ...
                shutil.copy(os.path.join(root, file), newname)
                assets.append(os.path.join('assets', rel_name))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--mkdocs-yml', type=str, default='/app/documentation/mkdocs.yml', help='Path to the mkdocs.yml file')
    parser.add_argument('--project-dir', type=str, default='/app/mkdocs2chm/project', help='Name of output directory')
    parser.add_argument('--exclude', nargs='+', default=["UNIX Installation", "UNIX User Guide"], help='Names of documents to exclude')
    ...
    print(f'Excluding the following documents: {args.exclude}')
    ...
    # assets, css = static_assets('/app/mkdocs2chm/assets', project=args.project_dir)
    assets, css = static_assets('/opt/mkdocs2chm/assets', project=args.project_dir)
    ...
    # Convert the mkdocs ToC to CHM ToC XML
