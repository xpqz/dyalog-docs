...
The Text object is used to write arbitrary text. It can be used in a [Form](form.md),
[SubForm](subform.md) or [Group](group.md) instead of a [Label](label.md). The main difference is that
a [Label](label.md) is implemented as a true window object
(thus consuming Windows resources). A Text object is not a window and consumes
no MS-Windows resources. However, a [Label](label.md) supports [DragDrop](../methodorevents/dragdrop.md) events and has various
...
[Points](../properties/points.md) is either a simple 2-column matrix
...
2. [Points](../properties/points.md) specifies more than one
    point. There are three possibilities:If Text is a scalar, its contents are written at each of the points in [Points](../properties/points.md).
    ...
    corresponding position in [Points](../properties/points.md).If Text is a matrix, each row of Text is written at the corresponding
    position in [Points](../properties/points.md).
3. If Text is a scalar, its contents are written at each of the points in [Points](../properties/points.md).
    This means that by enclosing a vector or matrix, you can draw a string or
    block of text at several locations.
4. If Text is a vector, each element of Text is written at the
    corresponding position in [Points](../properties/points.md).
...
[FCol](../properties/fcol.md) specifies the colour of the Text. For
...
intensities. If more than one text item is involved, [FCol](../properties/fcol.md) may be a vector which specifies the colour for each item separately. If so, its
length must be the same as the number of points specified by [Points](../properties/points.md).
...
[HAlign](../properties/halign.md) and [VAlign](../properties/valign.md) specify the horizontal and vertical alignment of the text respectively. They may
each be numeric scalars or vectors with the same length as the number of points
specified in [Points](../properties/points.md). See [HAlign](../properties/halign.md) and [VAlign](../properties/valign.md) for details.


When one or more of [FCol](../properties/fcol.md), [BCol](../properties/bcol.md),
[HAlign](../properties/halign.md) and [VAlign](../properties/valign.md) are vectors, the different components of Text are drawn using the corresponding
colours and alignments.


The value of the [Dragable](../properties/dragable.md) property
