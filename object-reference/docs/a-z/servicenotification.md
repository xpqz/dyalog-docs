...
<h1 class="heading"><span class="name">ServiceNotification</span><span class="command">Event 94</span></h1>
...
When the SCM requests a change of state, the APL interpreter responds by setting its state to the corresponding *pending* level (SERVICE_STOPPED_PENDING, SERVICE_RUNNING_PENDING or SERVICE_PAUSED_PENDING) and then generates a [ServiceNotification](../methodorevents/servicenotification.md) event.


It is the responsibility of the APL service to process this event, perform the appropriate application tasks, and then respond (to the SCM) by calling the [SetServiceState](../methodorevents/setservicestate.md) method to confirm that the service has reached the desired state.
