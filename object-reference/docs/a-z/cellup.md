...
If enabled, this event is reported when the user **releases** a mouse button down whilst over a cell in a [Grid](../objects/grid.md). This event is a companion to the [CellDown](../methodorevents/celldown.md) event and could be used to hide a pop-up which was displayed in response to the [CellDown](../methodorevents/celldown.md). The CellUp event performs no default action and may not be disabled.
...
An application **can** position the user on a particular cell in a [Grid](../objects/grid.md) by calling [CellDown](../methodorevents/celldown.md) event as a method, but it is recommended that a [CellMove](../methodorevents/cellmove.md) event is used instead.
