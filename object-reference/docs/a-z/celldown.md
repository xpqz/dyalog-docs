...
<h1 class="heading"><span class="name">CellDown</span><span class="command">Event 161</span></h1>
...
The default action is to generate a [CellMove](../methodorevents/cellmove.md) event which will then position the user on the new cell. This action can be prevented by returning 0 from the callback function, in which case the normally ensuing [CellMove](../methodorevents/cellmove.md) event will not occur.
...
An application **can** position the user on a particular cell in a [Grid](../objects/grid.md) by calling CellDown as a method, but it is recommended that a [CellMove](../methodorevents/cellmove.md) event is used instead.
