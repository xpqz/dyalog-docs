...
<h1 class="heading"><span class="name">CellChanged</span><span class="command">Event 164</span></h1>
...
Note that this event is reported **after** the change has taken place, and after the [Values](../properties/values.md) property has been updated. Furthermore, neither setting the event action code to `¯1` nor returning 0 from a callback function has any effect. If you wish to *validate* the new data you should use the [CellChange](../methodorevents/cellchange.md) (150) event instead.
...
If you want to update an individual cell under program control, you may call [CellChange](../methodorevents/cellchange.md), but not CellChanged, as a method.
